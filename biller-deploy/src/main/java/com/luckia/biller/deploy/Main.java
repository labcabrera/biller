...
public class Main {
	...
	public static void main(String[] args) {
		if (args.length != 3) {
			System.out.println(
					"Usage Main {generateBootstrap} {generateBills} {patch20140522}");
		}
		else {
	...
	/**
	 * DESTINATARIO: Egasa Hattrick S.A. Arzobispo Fabian y Fuero 17B 46009 Valencia
	 * España Tel: 961100793 NIF: A98359169
	 */
	private void updateEgasaInfo() {
		...
		TypedQuery<Company> query = entityManager.createQuery(
				"select e from Company e where e.name like :name", Company.class);
	...
	public void executePatch20140522() {
		...
		ranges.add(Range.between(new DateTime(2014, 1, 1, 0, 0, 0, 0).toDate(),
				new DateTime(2014, 1, 31, 0, 0, 0, 0).toDate()));
		ranges.add(Range.between(new DateTime(2014, 2, 1, 0, 0, 0, 0).toDate(),
				new DateTime(2014, 2, 28, 0, 0, 0, 0).toDate()));
		ranges.add(Range.between(new DateTime(2014, 3, 1, 0, 0, 0, 0).toDate(),
				new DateTime(2014, 3, 31, 0, 0, 0, 0).toDate()));
		ranges.add(Range.between(new DateTime(2014, 4, 1, 0, 0, 0, 0).toDate(),
				new DateTime(2014, 4, 30, 0, 0, 0, 0).toDate()));

		EntityManager entityManager = entityManagerProvider.get();
		TypedQuery<Company> queryCompanies = entityManager.createQuery(
				"select c from Company c where c.name like :name1 or c.name like :name2",
				Company.class);
		List<Company> companies = queryCompanies.setParameter("name1", "%Replay%")
				.setParameter("name2", "%Videomani%").getResultList();
		log.debug("Encontradas {} empresas", companies.size());

		TypedQuery<Long> query = entityManager.createQuery(
				"select s.id from Store s where s.parent in :companies", Long.class);
		...
		for (Company company : companies) {
			for (Range<Date> range : ranges) {
				LiquidationTask task = new LiquidationTask(company.getId(), range,
						injector);
				task.run();
			}
		}
