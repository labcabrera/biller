...
public class DashboardRestService {
	...
	public List<ChartModel> contractEvolutionAmount(
			@QueryParam("companyId") final Long companyId,
			@QueryParam("year") final Integer year) {
		Integer effectiveYear = year != null && year > 0 ? year
		...
		for (int i = 1; i < 13; i++) {
			DateTime from = new DateTime(effectiveYear, i, 1, 0, 0, 0);
			DateTime to = from.dayOfMonth().withMaximumValue();
			query.setParameter(1, companyId);
			query.setParameter(2, from.toDate());
			query.setParameter(3, to.toDate());
			BigDecimal partial = MathUtils
					.safeNull((BigDecimal) query.getResultList().iterator().next());
			String monthI18nKey = "month."
					+ StringUtils.leftPad(String.valueOf(i), 2, "0");
			list.add(new ChartModel(i18nService.getMessage(monthI18nKey),
					partial.doubleValue()));
		}
		return list;
	}
	...
	public List<ChartModel> companyStoreDistributionByMonth(
			@QueryParam("companyId") final Long companyId,
			@QueryParam("year") final Integer year,
			@QueryParam("month") final Integer month,
			@QueryParam("negative") final Boolean negative) {
		int effectiveYear = year != null ? year : new DateTime().getYear();
		int effectiveMonth = month != null ? month + 1 : new DateTime().getMonthOfYear();
		DateTime from = new DateTime(effectiveYear, effectiveMonth, 1, 0, 0, 0);
		DateTime to = from.dayOfMonth().withMaximumValue();
	...
	public List<ChartModel> companyStoreDistributionNegative(
			@QueryParam("companyId") final Long companyId,
			@QueryParam("year") final Integer year,
			@QueryParam("month") final Integer month) {
	...
	public List<ChartModel> companyStoreDistributionByYear(
			@QueryParam("companyId") Long companyId, @QueryParam("year") Integer year,
			@QueryParam("negative") Boolean negative) {
		Integer effectiveYear = year != null ? year : new DateTime().getYear();
		DateTime from = new DateTime(effectiveYear, 1, 1, 0, 0, 0);
		DateTime to = new DateTime(effectiveYear, 12, 31, 0, 0, 0);
		return companyDistribution(companyId, from, to, negative);
	}

	@GET
	@Path("company/storeDistributionAnual/negative")
	public List<ChartModel> companyStoreDistributionByYearNegative(
			@QueryParam("companyId") Long companyId, @QueryParam("year") Integer year) {
		return companyStoreDistributionByYear(companyId, year, true);
	}

	private List<ChartModel> companyDistribution(Long companyId, DateTime from,
			DateTime to, Boolean negative) {
		StringBuilder sb = new StringBuilder();
	...
	public Message<List<Liquidation>> liquidationPending(@QueryParam("n") Integer n,
			@QueryParam("p") Integer p) {
		EntityManager entityManager = entityManagerProvider.get();
	...
	private TypedQuery<ChartModel> getQuery(String sql, Integer year) {
		TypedQuery<ChartModel> query = entityManagerProvider.get().createQuery(
				String.format(sql, ChartModel.class.getName()), ChartModel.class);
		if (year != null && year != -1) {
