 ...
 * La aplicacion se gestiona a partir de un fichero *.conf estructurado en secciones con
...
public class SettingsManager {
	...
	public Properties getProperties(String sectionName) {
		if (sections == null) {
			throw new BillerException(
					"Service is not initialized. Call method load before");
		}
		else if (!sections.containsKey(sectionName)) {
			throw new BillerException(
					String.format("Missing section name %s", sectionName));
		}
		else {
	...
	public String getProperty(String sectionName, String property) {
		...
		if (!properties.containsKey(property)) {
			throw new BillerException(String.format("Missing property %s in section %s",
	...
	public <T> T getProperty(String sectionName, String property, Class<T> typeClass) {
		...
		if (String.class.isAssignableFrom(typeClass)) {
			...
			throw new InvalidParameterException(
					String.format("Class %s is not a valid type", typeClass.getName()));
		}
	}
	...
	public SettingsManager load() {
		...
		if (config.exists()) {
			try {
				...
				throw new BillerException("Error loading application config", ex);
			}
		}
		else {
			throw new BillerException(
					"Missing application config " + config.getAbsolutePath());
		}
		return this;
	}
	...
	public SettingsManager load(String classPathResource) {
		...
		InputStream configInputStream = classLoader
				.getResourceAsStream(classPathResource);
		Validate.notNull(configInputStream,
				String.format("Missing configuration file '%s' in classpath",
						Constants.APP_CONFIG_FILE));
		return load(configInputStream);
	}

	public SettingsManager load(InputStream inputStream) {
		BufferedReader reader = new BufferedReader(
				new InputStreamReader(inputStream, Charset.forName(CONFIG_FILE_CHARSET)));
		try {
			...
			while ((line = reader.readLine()) != null) {
				if (!StringUtils.isBlank(line) && !StringUtils.trim(line).startsWith("#")
						&& !StringUtils.trim(line).startsWith("//")) {
			...
			throw new BillerException("Error reading configuration", ex);
		}
		finally {
			try {
				reader.close();
