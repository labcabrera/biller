...
public class SystemCheckJob extends BaseJob {
	...
	private void checkRepositoryFolder() {
		String repositoryPath = injector
				.getInstance(Key.get(String.class, Names.named("repositoryPath")));
		if (repositoryPath == null) {
			alertService.handleAlert(
					"Missing configuration repository folder. Check app configuration");
		}
		else {
			File folder = new File(repositoryPath);
			if (!folder.exists()) {
				alertService.handleAlert(
						String.format("Missing repository folder '%s'", repositoryPath));
			}
			else if (!folder.canRead()) {
				alertService.handleAlert(String.format(
						"Invalid folder permissions '%s': cant read", repositoryPath));
			}
			else if (folder.canWrite()) {
				alertService.handleAlert(String.format(
						"Invalid folder permissions '%s': cant write", repositoryPath));
			}
		}
	}
}
