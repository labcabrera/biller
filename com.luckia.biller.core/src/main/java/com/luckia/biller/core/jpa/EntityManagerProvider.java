...
/**
 * Singleton encargado de gestionar la recuperación de los {@link EntityManager} de JPA. Para ello instancia un único EntityManagerFactory y asocia a cada thread que lo solicite su
 * propio EntityManager. <br>
 * De este modo si tenemos por ejemplo 4 threads operando de forma concurrente cada uno de ellos tendrá su propia instancia de EntityManager que funcionara independientemente de
	...
	public EntityManager get() {
		synchronized (entityManagerFactory) {
			if (entityManager != null && entityManager.get() != null && entityManager.get().isOpen()) {
				...
				LOG.trace("Generando EntityManager para el thread {}", Thread.currentThread().getName());
